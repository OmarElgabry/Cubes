...
| Maze | Given 2D array, starting from (0, 0), move to up, left, right and down. <br>Find the goal node by exploring all possible paths. Can you improve your solution (hint: using DP)? | ```Input:``` <br> array=<br> `. . X .` <br> `. X . G` <br> `. . . X` <br> `X . X X` <br><br> ```Output:``` <br>  [1, 3] | — | [C. Maze](http://codeforces.com/problemset/problem/378/C)
