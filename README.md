...
| Max Path | Given 2D array, starting from (0, 0), move only to right and down. <br>Find the maximum sum by exploring all possible paths. Can you improve your solution (hint: using DP)? | ```Input:``` <br> {{1, 5}, {2, 4}}<br> ```Output:``` <br>  10 | — | —
