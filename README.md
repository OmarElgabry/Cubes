...
| Zuma | Given array, and a _tnum_ number that will be inserted at index _tindex_. <br>If at there is three or more contiguous similar numbers, they should be destroyed(erased). <br>This rule is applied until there are no more three or more contiguous similar numbers. <br> Count the destroyed numbers. <br> _Initially, there is no three or more contiguous similar numbers._ | ```Input:``` <br> array={5, 4, 4, 2, 2, 4, 4, 5, 5, 1, 7, 6}, tnum=2, tindex=4<br> ```Output:``` <br>  10| O(N) | [B. Balls Game](http://codeforces.com/problemset/problem/430/B)
